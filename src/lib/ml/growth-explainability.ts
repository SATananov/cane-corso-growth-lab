...
function confidenceCopy(confidencePercent: number) {
  if (confidencePercent >= 82) {
    return "High app confidence for an educational tool. This means the entered data is complete and within the main age range of the prototype bridge, not that the result is clinically certain.";
  }
...
export function buildGrowthIntelligenceReport(
  ...
  return {
    ...
    modelExplanation: [
      {
        ...
        detail:
          "The regression research explains growth as a curve. The app uses that idea to estimate the expected weight point for the selected age and adult reference.",
      },
      {
        title: "Classification layer",
        detail:
          "The classification research inspires the review zones. The app translates the current point into balanced, watch or attention signals.",
      },
      {
    ...
    safetyBoundary:
      "This report is generated by an educational machine learning prototype. It is not veterinary advice, not a diagnosis and not a replacement for professional examination.",
  };
}
