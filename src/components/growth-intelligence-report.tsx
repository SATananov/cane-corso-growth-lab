        ...
        {report.keyFactors.map((factor, index) => (
          <article key={factor.label} className="min-w-0 rounded-3xl border border-stone-700 bg-white/[0.03] p-4">
            <div className="flex items-start justify-between gap-3">
              <p className="min-w-0 text-sm font-semibold leading-5 text-stone-300">{t.factors[index]}</p>
              <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[0.68rem] font-semibold ${factorToneStyles[factor.tone]}`}>{t.tones[factor.tone]}</span>
            </div>
