...
const copy: Record<LanguageCode, {
  ...
  bg: {
    ...
    notebookConnected: "Notebook основата е свързана с приложението",
    bestRegression: "Най-добър regression модел",
    bestClassifier: "Най-добър classification модел",
    methodologyAssets: "Методологични файлове",
    notebooks: "notebook-и",
    dataFiles: "файла с данни",
    figures: "фигури",
    regressionEvidence: "Regression доказателства",
    growthCurveModels: "Модели за крива на растеж",
    classificationEvidence: "Classification доказателства",
    ...
    assets: { notebooks: "Notebook-и", data: "Данни", figures: "Фигури", appUse: "Употреба в приложението", referenceFile: "Референтен файл" },
  },
  it: {
    methodologyStatus: "Stato metodologia",
    notebookConnected: "La base notebook è collegata all’app",
  ...
  return (
    <div className="grid gap-6">
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard label={t.bestRegression} value={mlFoundationSummary.regressionBestModel} hint={`R² ${formatMetric(mlFoundationSummary.regressionBestR2)}`} />
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <ModelTable eyebrow={t.regressionEvidence} title={t.growthCurveModels} bestLabel={t.bestCurrentResult} rows={regressionResults.map((result) => ({ name: result.model, type: result.geometry, metricOne: `MAE ${formatMetric(result.mae)}`, metricTwo: `R² ${formatMetric(result.r2Score)}`, isBest: result.model === bestRegression?.model }))} />
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        <AssetPanel title={t.assets.notebooks} assets={notebookAssets} labels={t.assets} />
        <AssetPanel title={t.assets.data} assets={dataAssets} labels={t.assets} />
        <AssetPanel title={t.assets.figures} assets={figureAssets} labels={t.assets} />
      </section>
    </div>
  );
}
...
type AssetPanelProps = { title: string; assets: ResearchAsset[]; labels: { appUse: string; referenceFile: string } };
function AssetPanel({ title, assets, labels }: AssetPanelProps) {
  return (
    <section className="rounded-[2rem] border border-amber-200/10 bg-white/[0.035] p-6">
