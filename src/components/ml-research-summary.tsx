...
> = {
  ...
  bg: {
    ...
    safetyFallback:
      "Методологията показва ориентировъчни сигнали и не доказва диагноза, породна чистота, родословие, генетичен произход или официален статус.",
    assets: {
      notebooks: "Notebook-и",
      data: "Данни",
      figures: "Фигури",
  ...
  it: {
    ...
    safetyFallback:
      "La metodologia mostra segnali orientativi e non prova diagnosi, purezza di razza, pedigree, origine genetica o status ufficiale.",
    assets: {
      notebooks: "Notebook",
      data: "Dati",
      figures: "Figure",
  ...
  return (
    <div className="grid gap-6">
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SummaryCard
          label={t.methodologyStatus}
          value={mlFoundationSummary.status}
          hint={t.notebookConnected}
        />

        <SummaryCard
          label={t.bestRegression}
          value={mlFoundationSummary.regressionBestModel}
          hint={`R² ${formatMetric(mlFoundationSummary.regressionBestR2)}`}
        />

        <SummaryCard
          label={t.bestClassifier}
          value={mlFoundationSummary.classificationBestModel}
          hint={`F1 ${formatMetric(mlFoundationSummary.classificationBestF1)}`}
        />

        <SummaryCard
          label={t.methodologyAssets}
          value={`${mlFoundationSummary.notebookCount} ${t.notebooks}`}
          hint={`${mlFoundationSummary.dataFiles} ${t.dataFiles} / ${mlFoundationSummary.figures} ${t.figures}`}
        />
      </section>

      <section className="grid gap-5 lg:grid-cols-2">
        <ModelTable
          eyebrow={t.regressionEvidence}
          title={t.growthCurveModels}
          bestLabel={t.bestCurrentResult}
        ...
        <ModelTable
          eyebrow={t.classificationEvidence}
          title={t.reviewZoneModels}
          bestLabel={t.bestCurrentResult}
      ...
      <section className="rounded-[2rem] border border-amber-200/10 bg-amber-300/[0.06] p-6">
        ...
        <p className="mt-3 max-w-4xl text-sm leading-7 text-stone-300">
          {language === "en" ? mlFoundationSummary.safetyNote : t.safetyFallback}
        </p>
      </section>
