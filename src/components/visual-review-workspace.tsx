...
import { photoReadinessRules, type PhotoReadinessLevel } from "@/lib/ml/photo-comparison-criteria";
...
const readinessRuleCopy: Record<
  LanguageCode,
  Record<PhotoReadinessLevel, { label: string; meaning: string; action: string }>
> = {
  en: {
    accepted: {
      label: "Accepted",
      ...
      action: "Allow comparison, geometry overlay and visual match explanation.",
    },
    limited: {
      label: "Limited",
      meaning:
    ...
    rejected: {
      ...
      action: "Block the visual match score and ask for a new image following the photo guide.",
    },
  },
  bg: {
    accepted: {
      label: "Подходяща",
      meaning:
        "Снимката покрива указанията достатъчно добре за надеждно визуално геометрично сравнение.",
      action: "Разреши сравнение, геометрично наслагване и обяснение на визуалното сходство.",
    },
    limited: {
      label: "Ограничена",
      meaning:
        "Снимката може да позволи частичен преглед, но перспектива или видимост намаляват увереността.",
      action:
        "Покажи ясно предупреждение за надеждност и обясни кои части от сравнението са ограничени.",
    },
    rejected: {
      label: "Неподходяща",
      meaning:
        "Снимката не покрива критериите за сравнение с еталонната геометрия на Cane Corso.",
      action: "Блокирай визуалната оценка за сходство и поискай нова снимка по указанията.",
    },
  },
  it: {
    accepted: {
      label: "Accettata",
      meaning:
        "La foto segue la guida abbastanza bene per uno spazio affidabile di confronto geometrico visuale.",
      action: "Consenti confronto, sovrapposizione geometrica e spiegazione del match visuale.",
    },
    limited: {
      label: "Limitata",
      meaning:
        "La foto può supportare una revisione parziale, ma prospettiva o visibilità riducono la confidenza.",
      action:
        "Mostra un avviso forte di affidabilità e spiega quali parti del confronto sono limitate.",
    },
    rejected: {
      label: "Respinta",
      meaning:
        "La foto non soddisfa i criteri per il confronto con la geometria di riferimento Cane Corso.",
      action: "Blocca il punteggio visuale e richiedi una nuova immagine seguendo la guida fotografica.",
    },
  },
};
          ...
          <table className="min-w-full divide-y divide-stone-700 text-left text-sm">
            ...
            <tbody className="divide-y divide-stone-800 bg-black/15">
              {photoReadinessRules.map((rule) => {
                const localizedRule = readinessRuleCopy[language][rule.level];
                ...
                return (
                  <tr key={rule.level}>
                    ...
                    <td className="px-4 py-4 text-stone-300">
                      {localizedRule.meaning}
                    </td>
                    <td className="px-4 py-4 text-stone-400">
                      {localizedRule.action}
                    </td>
                  </tr>
                );
              })}
